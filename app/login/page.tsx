...
export default function Login(){
  ...
  return <main className="shell"><nav className="nav"><a className="brand" href="/">BREAKTHROUGH</a></nav><section className="formPage"><div className="eyebrow">Welcome back</div><h1>Log in</h1><form onSubmit={submit}><input className="input" placeholder="Email" type="email" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email"/><input className="input" placeholder="Password" type="password" value={password} onChange={e=>setPassword(e.target.value)} required autoComplete="current-password"/><button className="btn primary" type="submit" disabled={loading}>{loading?'Signing in…':'Log in'}</button></form>{msg&&<div className="notice" style={{color:'#ffb4b4'}}>{msg}</div>}<p className="muted">New here? <a href="/signup">Create an account</a></p></section></main>
}
